...
2. **PR 内の workflow 自己改変**: 通常の required checks は PR ブランチ側の定義なので、
   それだけなら骨抜きにできる。本キットは `pull_request_target` の `workflow-integrity` を
   required にし、base branch のコードだけで PR head をデータとして検査する。信頼済み
   workflow・検査器・CI workflow 全体の変更を拒否する。ただしrequired checkはworkflowを
   識別せずjob名だけを見るため、PR側の同名ダミーjobという偽装余地がある。したがって
   `.github/CODEOWNERS` とcode owner review必須設定を併用し、信頼境界の変更はPR作成者とは
   別の人間承認なしに合流できないようにする。

したがって README 等の「技術的ブロック」は、**現在のセッションの操作経路
（PreToolUse フック）に対しては真**だが、門の実装ファイルへの書込みには開いている
（存在の消失は `missing-required`・単体の骨抜きは guard-corpus・workflow自己改変は
`workflow-integrity`＋必須code owner reviewが検出する。残余はオラクル同時書換）。キットが規定する防衛の最外周はリポジトリ設定（Step 9 ④ required checks——
...
- **`scripts/check_bootstrap.py`**（pre-commit の `check-bootstrap`——`files: ^\.guardrails/BOOTSTRAP\.md$`
  ...
  - `bootstrap-false-done`: **✅ の Step ごとに、検証可能なアサーションをその場で再実行**
    ...
    **不在の断定は両系統の確定回答が揃った時のみ**（v2.36 是正——片系統が照会不能のまま
    ...
    強制線は **4コアジョブ**（`checks`・`red-first`・`commit-msg-history`・`workflow-integrity`）
    と採用した全言語別job（v2.50 是正）: 「一部はローカルでも走るから重複」はローカルフックが動く経路にしか
    成立せず、CI を最終防衛線とする主張（§5）の対象経路（Web 編集・フック未導入マシン）では、
    required でなければ赤のままマージできる。workflow自己改変、列固有テスト、ruleset bypass、
    旧来保護のadmin非強制、code owner review無効も未保護として失敗する。回帰は
    `check_bootstrap.py --verify-scenarios`（pre-commit の
...
2. **1 Phase（1 Step）= 1 ブランチ = 1 PR**。複数をまとめない——どのゲートがどの変更を
   検証したのか、後から追えなくなる。**既定ブランチへの直接 push は禁止**。PR の
   required checks が、Web 編集・フック未導入マシンの変更を**合流前に止める**唯一の防壁で
   あり、`push` の CI は合流後の事後検知なので代替にならない。
   （§11 の Step のコミット粒度は `bootstrap-multi-flip` が ✅ 化を1コミット1Stepに制限。
   PR 経由の強制は Step 9 ④ のサーバー側「PR 必須＋required checks」が担う — §3.5）
...
| 32 | guardコーパス性能是正（プロセス起動回数削減） | §2 §7.7 | ✅（v2.22 同梱） |
| 33 | guardフックのbash→Python移行 | §2 | ✅（v2.23 同梱） |
| 34 | 残り4フックのPython移行＋post-edit呼出し是正 | §2 | ✅（v2.24 同梱） |
| 35 | NONDETERMINISM-EXEMPT（非決定性テストの免除） | §9.5 | ✅（v2.25 同梱） |
| 36 | 所有権ガード回帰シナリオ | §2c | ✅（v2.29 同梱） |
| 37 | commit-msg系ゲートのCI履歴再実行 | §3.4 §5 | ✅（v2.30 同梱） |
| 38 | binding-dead-path（パス型バインディングのドリフト検出） | §3.3 §12.7 | ✅（v2.32 同梱） |
| 39 | violation ledger（門が止めた事象の記録） | §3.6 | ✅（v2.34 同梱） |
| 40 | Step 9外部設定の実測検証 | §3.5 §11 Step 9 | ✅（v2.35 同梱） |
| 41 | 導入済みリポジトリの更新経路 | §3.3 §11 | ✅（v2.38 同梱） |
| 42 | fix-without-testのインラインテスト対応 | §3.4 | ✅（v2.39 同梱） |
...
| 51 | PR 必須契約の統一＋外部設定監査 | §3.5 §11 Step 9 | ✅（v2.49 同梱） |
| 52 | workflow自己改変防御＋既定ブランチ/required/bypass/台帳監査の是正 | §3.5 §5 §10 §11 Step 9 | ✅（v2.50 同梱） |
| 53 | required check同名job偽装の封鎖（CODEOWNERS＋必須人間レビュー） | §2e §3.5 §5 §11 Step 9 | ✅（v2.51 同梱） |
...
- **プロンプト分割の不採用（判断ごと記録——再提案ループ防止）**: プロンプトを複数に
  ...
  **1 Step = 1 コミット = 1 ブランチ = 1 PR**として存在する（本 Phase の門が機械検証する
  のはコミット粒度、PR 経由は Step 9 ④ のサーバー設定が強制）——
...
### Phase 51 — v2.49 同梱 ✅（PR 必須契約の統一＋外部設定監査）（G7/G9/G10）

- 発見: 正本は全 Step を `1ブランチ=1PR` とした一方、新規向けプロンプトだけが
  「Step 9 以降は1PR」と要約し、Step 0〜8 の直接 push を許す読み方が成立していた。
- 判定: PR は全廃できない。`red-first` と `commit-msg-history` は PR の base/head を入力にし、
  required checks は Web 編集・フック未導入マシンの変更を既定ブランチへ**入る前**に止める。
  `push` は壊れた合流後の事後検知であり、この予防を代替しない。したがって全 Step・
  全トピックを `1コミット=1ブランチ=1PR` へ統一し、既定ブランチへの直接 push を禁止した。
- 実装: `verify_required_checks` を3コアジョブの登録確認から「PR 必須＋3コアジョブ」の確認へ
  拡張。rulesets の `pull_request` と旧来ブランチ保護の required pull request reviews を
  両系統で照会し、確認できて不在は失敗・照会不能は表示して素通し、という既存契約を維持。
  `--verify-scenarios` に「checks は揃うがPR必須なし」を追加し、直接 push の再開を回帰化した。

### Phase 52 — v2.50 同梱 ✅（workflow自己改変防御＋監査のfail-open是正）（G7/G9/G10）

- `pull_request_target` の信頼済みworkflowは base branch のコードだけを実行し、PR head を
  checkoutせず Git blob として読む。信頼済みworkflow・検査器・`guardrails-ci.yml` 全体を
  base/headバイト一致で固定し、コアjob名変更・`run: true`・言語別jobの骨抜きを合流前に拒否する。
- required contexts は4コア（`checks`・`red-first`・`commit-msg-history`・`workflow-integrity`）と
  採用した全言語別job。ruleset bypass、旧来保護のadmin非強制、言語job未登録も監査で失敗する。
- CIのpush triggerとred-firstの既定baseをブランチ名非依存にし、YAML job抽出の誤認を修正。
  Phase台帳と見出しのドリフトもhard検査にした。
- workflow更新時は人間がPRをレビューし、`workflow-integrity` required指定を一時解除して
  **同じPRをmerge後ただちに再有効化**する。既定ブランチへの直接pushは例外にしない。

### Phase 53 — v2.51 同梱 ✅（required check同名job偽装の封鎖）（G7/G9/G10）

- 発見: GitHubのrequired status checkはworkflow/eventを識別せずjob名だけを見る。同じGitHub
  Actions appからPR側の別workflowが同名jobを報告できるため、`workflow-integrity` 単独を
  信頼境界とするv2.50の結論は過大だった。
- 是正: `.github/CODEOWNERS` でworkflowディレクトリ全体・integrity検査器・CODEOWNERS自身を
  PR作成者とは別の人間ownerへ割り当てる。ruleset/旧来保護ではcode owner review必須・
  admin適用・bypass無しを同時に要求し、required checksの期待送信元をGitHub Actions Appへ
  固定する。check-bootstrapはローカル定義と外部設定を実測する。ruleset APIが権限不足で
  `bypass_actors` を省略した場合は0人と誤認せず「検証不能」とする。
- CODEOWNER承認後の追加pushで古い承認を再利用できないよう、`dismiss stale reviews` または
  `require last push approval` も必須とする。どちらも無い設定は未保護として失敗する。
- 境界: CODEOWNERはAI・アプリ・PR作成者本人にしない。人間ownerを用意できない個人運用では、
  GitHub Freeのstatus checkだけでworkflow自己改変を完全には封鎖できないためStep 9を✅にしない。
...
- **合流の門（GitHub Merge Queue——調査⑤・企業実証: Rust bors / Uber SubmitQueue /
  ...
  **または合流起因の main 赤を1回でも実測**した時。守る対象 = マージスキュー（個別に緑の
  ...
  required checks を前提に Merge Queue を有効化。現行の部分防御 = CI の `push`
...
**§10 冒頭の実行規律をそのまま適用**（順序固定・全 Step で
1 Step = 1 コミット = 1 ブランチ = 1 PR・既定ブランチへの直接 push 禁止・違反注入必須・
虚偽 ✅ 禁止・途中でターンを終えない）。
...
- 完了条件: ① 正常 PR で全ジョブ緑 ② **GitHub の Web エディタから**違反を1件コミット
  ...
  **ジョブ赤**（exit 1）・正しい fix で証明1行＋緑・`RED-FIRST-EXEMPT`（理由あり）で
  免除1行・理由なし EXEMPT は免除不成立で赤のまま。仕上げにブランチ保護 / rulesets で
  **「変更はPR経由」を必須化**し、required checks へ
  **4コアジョブ（`checks`・`red-first`・`commit-msg-history`・`workflow-integrity`）と採用した
  全言語別job**を登録する（required の完成は
  リポジトリ設定まで — v2.9。PR 必須なのは、これらを既定ブランチへの合流前に効かせ、
  Web 編集・フック未導入経路を予防するため。`push` は合流後の検知で代替にならない。
  4コアに加えて全言語別jobを要求する理由は
  §3.5——Web 編集・フック未導入経路では CI ジョブが唯一の強制点。**登録の有無は
  check-bootstrap が `gh api` で実測検証する**——検証不能環境は表示して素通し・検証できて
  不在なら本 Step は ✅ にできない — §3.5・v2.35〜v2.51）。登録は **rulesets 側を推奨**
  （CI の GITHUB_TOKEN で照会できるのは rulesets のみ＝CI 再監査が確定判定になる — v2.36）。
  bypass主体を置かず、旧来保護を使う場合はadminにも強制する。workflow更新だけは人間レビュー後に
  `workflow-integrity` のrequired指定を一時解除し、同じPRをmerge後ただちに再有効化する。
  `.github/CODEOWNERS` のplaceholderをPR作成者とは別の人間ownerへ置換し、workflow群・
  integrity検査器・CODEOWNERS自身へのcode owner reviewもサーバー側で必須化する。
  追加pushで承認を使い回せないよう `dismiss stale reviews` または `require last push approval`
  も有効にする。
  required checksは「any source」にせずGitHub Actions Appを期待送信元に指定する。rulesetの
  bypass一覧は書込み権限がないトークンには返らないため、Step 9完了時は管理権限のある`gh`で
  実測する（CIで項目が省略された場合は0人と断定しない）。
