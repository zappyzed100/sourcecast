...
- [ ] 配信先の実アップロードクライアントを実装する（YouTube×2・Podcast×2・Amazon系×2。
  `distribution_ledger.py`の`dispatch()`へ渡す`publish_fn`が現状テストのフェイクのみ。
  各チャンネルの開設はHUMAN_TASKS.md Phase 9で依頼中——開設完了が着手の前提） `blocked`

### 二言語（JP / EN）展開（2026-07-24決定）

配信チャンネルを日本語版・英語版へ分割する決定に伴う実装タスク。収集・権利判定・
題材選出・スライド生成・FFmpeg合成は言語非依存でそのまま流用できるが、以下は
言語ごとの分岐か新規実装が要る。

- [ ] 英語TTSとして**Kokoro TTS**（Apache-2.0・ローカル・82Mパラメータ）を導入する
  （2026-07-24決定。VOICEVOXは日本語専用で流用不可——`media/voicevox.py`が現状唯一のTTS。
  VOICEVOXと同じ「ローカルで動かして叩く」構造なので、`media/`配下へ対称なクライアントを
  置き、言語で使い分けるディスパッチを噛ませる。`config/external_tools.yaml`へ
  バージョン・取得元URL・SHA-256を記録するのもVOICEVOX/FFmpegと同じ扱い） `next`
- [ ] サイトを言語別パスへ分割する（2026-07-24決定: `itsuwawa.com/ja/`・`/en/`。
  サブドメイン/別ドメインは不採用——Pagesプロジェクト1つ・ドメイン1つのまま運用でき、
  SEO上も有利なため。現在の日本語コンテンツはルート直下にあるのでリダイレクトが要る。
  Pagefindのインデックスも言語別に分ける） `backlog`
- [ ] RSSフィードを言語別に生成する（`apps/site/src/pages/feed.xml.ts`が
  `<language>ja</language>`・タイトル・説明文をハードコードしており、`publish/`配下には
  言語という概念自体が無い。フィードURLは上のパス構造に従う） `backlog`
- [ ] 台本生成を2言語対応にする（同一の主張台帳から各言語の台本を作る。無料モデル方針の
  ため費用は増えないが実行時間は2倍。翻訳するのか各言語で生成するのかは着手時に決める） `backlog`
- [ ] 日本語固有名詞の英語表記変換を用意する（Phase 6〜7で構築した読み辞書7ソースは
  日本語音声のための**読み**であり英語版では無用——代わりに「徳川家康 → Tokugawa Ieyasu」の
  ような英語表記が要る。Wikidataの多言語ラベルが一次候補） `backlog`
