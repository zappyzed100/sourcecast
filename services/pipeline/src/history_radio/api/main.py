...
async def lifespan(app: FastAPI) -> AsyncGenerator[None]:
    """起動時に前回プロセスの中断ジョブを検出する（development-plan.md Phase 12
    タスク4）——単一プロセス前提のため、起動時点でstatus='running'のジョブが
    残っていること自体が異常終了の証拠になる（jobs/recovery.py参照）。
    """
    del app
    recover_orphaned_jobs(get_session_maker())
    yield


app = FastAPI(title="history-radio admin API", version="1", lifespan=lifespan)
