...
読み辞書（人名・地名・元号・官職）の設計・出典・ライセンス方針は本書 §8 が正本。
VOICEVOXのナレーション品質は読み仮名の正確さに直結する（仕様書§9.2「読み上げ困難な
固有名詞には読み仮名を付ける」）ため、音声生成タスクの前段として実装する。

* [ ] 読み辞書の解決器を実装する（§8の解決順序: 手動修正辞書 → 元号・官職辞書 →
  Wikidata/NDL → 地名辞書 → JMnedict → Sudachi → 未解決は公開前レビューへ）。
  検証: 各層のnamedテスト＋「どの層でも解決できない語が`unresolved`として
  公開ゲートに届く」ことを固定する。LLMに読みを推測させない。
* [ ] 外部辞書の取得スクリプトと `THIRD_PARTY_NOTICES.md` を実装する（辞書本体は
  コミットせず公式配布元からダウンロード。ソースごとに `source_url`/`license` を保持）。
  検証: 取得後に件数・ハッシュを検査し、ライセンス表記の欠けたソースが登録できない。
* [ ] 手動修正辞書 `config/readings/manual.yaml` の形式（surface/reading/kind/context/
  confidence）をSchema検証し、文脈依存の複数読み（例: 判官=ほうがん/はんがん）を表現できるようにする。
  検証: 不正エントリを起動時に拒否し、同一表記の文脈別読みが正しく引ける。
