...
- [voltagent/awesome-design-md](https://github.com/voltagent/awesome-design-md) は74ブランド分の DESIGN.md 参照資料として `upstream/design-md/` に submodule + sparse-checkout でローカル配置しています(UI の見た目・方向性の検討時に参照。選定理由は [.upstream/sources.yaml](.upstream/sources.yaml))。
...
│  └─ design-md/
│     └─ awesome-design-md/        # submodule + sparse-checkout(74ブランドの DESIGN.md 参照資料)
├─ scripts/
...
│  └─ workflows/
│     └─ update-upstreams.yml      # 週1の上流監視(パス差分のsubmodule更新PR + ui-ux-pro-max再ベンダーPR)
└─ .upstream/
...
**(b) emil UI スキル / awesome-design-md**(検知: update-upstreams.yml の submodule 更新 PR)——PR を取り込むだけ。

**(c) ui-ux-pro-max**(検知: update-upstreams.yml の再ベンダー PR)——PR を取り込むだけ。
週次を待たずに手元で更新したい時は:
...
すべての上流を**週1・同じタイミング(月曜 06:00 UTC)**でチェックします。

| 上流 | 取り込み | 更新 |
|---|---|---|
| `zappyzed100/guardrails-kit` | submodule(全体) | **Dependabot** が最新 commit への更新 PR を作成 |
| `emilkowalski/skills` | submodule + sparse-checkout | **update-upstreams.yml** が監視し、採用 Skill のパスに差分がある場合のみ更新 PR を作成 |
| `voltagent/awesome-design-md` | submodule + sparse-checkout(DESIGN.md のみ) | **update-upstreams.yml** が監視し、`design-md/` に差分がある場合のみ更新 PR を作成 |
| `nextlevelbuilder/ui-ux-pro-max-skill` | ベンダーコピー(`.claude/skills/`) | **update-upstreams.yml** が `scripts/revendor_uipro.py` を実行し、差分がある場合のみ再ベンダー PR を作成 |

`update-upstreams.yml`(submodule 側)の判定ロジック:

1. 現在固定中の submodule SHA と上流 `main` の最新 SHA を比較
2. 同一なら何もしない
3. 異なる場合、`git diff --quiet <current> <latest> -- <監視パス群>` で監視パスの差分を確認(パスの正本は [.upstream/sources.yaml](.upstream/sources.yaml) と workflow の `watch_paths`)
4. 監視パス外の変更のみなら何もしない。差分があれば submodule SHA を更新する PR を作成

Dependabot は全 submodule を対象にするため、`dependabot.yml` の `ignore` で
`emilkowalski-skills` と `awesome-design-md` を除外し、この2つは上記 Action のみで管理しています。

## 新しい上流 / Skill を採用するとき

1. `.upstream/sources.yaml` に選定結果を追記
2. sparse-checkout 対象なら `scripts/setup-upstreams.ps1` のパス定数と
   `update-upstreams.yml` の `watch_paths`(および `scripts/install_workbench.py` の
   `SUBMODULES`)を同期
